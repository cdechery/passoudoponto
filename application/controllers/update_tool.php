<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Update_tool extends CI_Controller {
		...
		public function __construct() {
			...
			if( "###changethis###"==$this->params['update_tool']['password'] ) {
		...
		public function confirm_settings() {
			...
			if( $pwd != $this->params['update_tool']['password'] ) {
			...
			if( $this->params['update_tool']['skip_files']=='' ) {
				$settings .= "NONE";
			} else {
				$settings .= "<ul><li>";
				$settings .= implode( "</li><li>", $this->params['update_tool']['skip_files'] );
				$settings .= "</li></ul>";
			}
		...
		public function do_update() {
			...
			if( $this->params['update_tool']['skip_files']!=''
				 && count($this->params['update_tool']['skip_files']) ) {
				$this->base_skip_files = array_merge($this->base_skip_files,
					$this->params['update_tool']['skip_files']);
			}
			...
			if( $master_file_ok ) {
				...
				if ($res === TRUE) {
					...
					$this->recurse_copy($base."/update/doacoes-master", $base);
					$output .= "OK<br>";

					$output .= "Checking for changes on <i>site_params.php</i>: ";

					include($base."/application/config/site_params.php");
					$old_params = $config['site_params'];
					include($base."/update/doacoes-master/application/config/site_params.php");
					$new_params = $config['site_params'];

					$dif = array_diff_key($old_params, $new_params);
					if( count($dif) ) {
						$output .= "<u>There are changes (new stuff) on <i>site_params.php</i></u>. You must merge the new and current versions manually.";
					} else {
					...
					if( $delete ) {
						...
						@delTree($base."/update/doacoes-master");
						$output .="OK";
					}
