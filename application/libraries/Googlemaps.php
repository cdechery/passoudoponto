...
class Googlemaps {
	...
	var $custom_js_global			= FALSE;					// Custom Javascript code to include Globally					
	var $custom_js_init				= FALSE;					// Custom Javascript code to include inside the initialize function 
	...
	function add_marker($params = array())
	{
		...
		if ($marker['infowindow_content']!="") {
			...
			google.maps.event.addListener(marker_'.$marker_id.', "click", function(event) {
				...
				marker_'.$marker_id.'.infowindow.setContent("<div class=\"iw_loading_box\"><img src=\"'.img_url('ajax-loader.gif').'\" class=\"iw_loading_img\"></div>");
	...
	function create_map()
	{
		...
		if ($this->custom_js_global) { 
			$this->output_js_contents .= $this->custom_js_global;
		}
		...
		if( $this->custom_js_init ) {
			$this->output_js_contents .= $this->custom_js_init;	
		}
		...
		function createMarker(markerOptions, _new) {
			...
			if( _new ) {
				...
				// clear any New Marker infowindows, to always keep only one open
				...
				//alert("Zoom atual:"+map.getZoom());
				
				//add the newly created marker to a new_markers array
	...
	function get_address_from_lat_long($lat, $long, $attempts = 0) {
		
		$status = '';
		$address = '';
		
		if ($this->https) { $data_location = 'https://'; } else { $data_location = 'http://'; }
		$data_location .= "maps.google.com/maps/api/geocode/json?latlng=".$lat.",".$long."&sensor=".$this->sensor;
		...
		if( !$data ) {
			...
			if( $json_data->status == "OK" ) {
				$address = $json_data->results[0]->address_components[1]->long_name.", ".$json_data->results[0]->address_components[0]->short_name;
				$status = "OK";
			} else {
	...
	function get_street_view_img( $lat, $long, $params = array() ) {
		...
		if( count($params) == 0 ) {
			$params = array("location"=>$lat.",".$long,
							"sensor"=>"false",
							"heading"=>180,
							"size" => "600x400");
		}

		$data_location = "http://maps.googleapis.com/maps/api/streetview?".http_build_query($params);
		$ctx = stream_context_create(array( 
   				 'http' => array( 
        			'timeout' => 1 
        	 	) 
		...
		return array("status"=>$status, "file_name"=>$file_name);
	}
